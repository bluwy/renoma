      ...
      return {
        ...
        'Document > Object > Member[name.value="dependencies"] > Object': (
          ...
          const dependencies = new Set(
            node.members
              .map((m) => {
                if (m.name.type === 'String' && m.value.type === 'String') {
                  dependencyToNode.set(m.name.value, m)
                  return m.name.value
                } else {
                  return ''
                }
          ...
          for (const m of node.members) {
            if (m.name.type !== 'String' || m.value.type !== 'String') continue

            const dependency = m.name.value
